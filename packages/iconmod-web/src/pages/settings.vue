...
import { categories, collectionList } from '../data'
import { isExcludedCategory, toggleExcludedCategory } from '../store'

const categorizedCollections = computed(() => categories.value.map(category => ({
  name: category,
  type: 'normal' as PresentType,
  collections: collectionList.value.filter(collection => collection.category === category),
})))
</script>
        ...
        <div v-for="c of categorizedCollections" :key="c.name" mb-10>
          <div flex py1 px2 break-inside-avoid>
            ...
            <button
              ...
              @click="toggleExcludedCategory(c.name as string)"
            />
          </div>
