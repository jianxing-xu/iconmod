...
export function createAPIv2SearchResponse(q: Record<string, string>): number | APIv2SearchResponse {
  ...
  // Get limits
  if (v2Query.limit) {
    const limit = Number.parseInt(v2Query.limit)
    if (!limit) {
      return 400
    }
  ...
  if (v2Query.min) {
    const limit = Number.parseInt(v2Query.min)
    if (!limit) {
      return 400
    }
    params.limit = Math.max(minSearchLimit, Math.min(limit, maxSearchLimit))
  ...
  let start = 0
  if (v2Query.start) {
    start = Number.parseInt(v2Query.start)
    if (Number.isNaN(start) || start < 0 || start >= params.limit) {
      return 400
    }
  }
